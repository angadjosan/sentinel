        ...
        if event.type == "tool_call":
            ...
            if event.tool_name == "emit_finding" and isinstance(event.result, dict):
                ...
                if fp not in suppressed_fps:
                    # Only set node_id if the node actually exists in this graph
                    # — plan scans have no graph nodes, so a synthetic id like
                    # "file:plan.txt" would point findings.node_id at nothing.
                    from .models import Node as _Node
                    raw_node_id = finding_data.get("node_id")
                    if raw_node_id:
                        node_exists = await db.get(_Node, {"graph_id": graph.id, "id": raw_node_id})
